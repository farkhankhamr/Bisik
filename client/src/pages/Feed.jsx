...
export default function Feed() {
    ...
    const mixedFeed = useMemo(() => {
        ...
        // 2. Mixing (only if ALL)
        if (filterChip !== 'ALL') {
            // Simple concatenation if sorted or filtered to one type
            const combined = [...filteredPosts, ...filteredIntel];
            // Sort simply by created_at desc if we just concat
            return combined.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
        }

        // Logic: 1 intel per 5 curhats. First 2 must be curhat.
        const mixed = [];
        let pIdx = 0;
        let iIdx = 0;
        let sinceLastIntel = 0;

        // Force first 2 posts to be curhat (if available)
        while (pIdx < filteredPosts.length && mixed.length < 2) {
            mixed.push(filteredPosts[pIdx++]);
            sinceLastIntel++;
        }

        // Now mix
        while (pIdx < filteredPosts.length || iIdx < filteredIntel.length) {
            // Condition to insert INTEL: 
            // - We have intel left
            // - We need to space it out (say every 5 posts) OR we ran out of posts
            // But prompt says "1 intel per 4-6 curhat cards".
            // Let's try inserting intel every time `sinceLastIntel` hits 5

            if (iIdx < filteredIntel.length && (sinceLastIntel >= 5 || pIdx >= filteredPosts.length)) {
                mixed.push(filteredIntel[iIdx++]);
                sinceLastIntel = 0;
            } else if (pIdx < filteredPosts.length) {
                mixed.push(filteredPosts[pIdx++]);
                sinceLastIntel++;
            } else {
                // Ran out of posts, but have intel (handled in first if clause generally, but simple loop safety)
                if (iIdx < filteredIntel.length) {
                    mixed.push(filteredIntel[iIdx++]);
                } else {
                    break;
                }
    ...
    }, [posts, intel, myPosts, activeTab, filterChip]);
