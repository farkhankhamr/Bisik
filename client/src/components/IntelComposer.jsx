...
import { X, Tag, AlertTriangle, Clock, MapPin } from 'lucide-react';
...
export default function IntelComposer({ onClose }) {
    ...
    const [step, setStep] = useState('select'); // select, deal, headsup
    ...
    const handleSelectHeadsUpType = (type) => {
        setHeadsUpType(type);
        setStep('headsup-form');
        // Prefill content based on template
        const templates = {
    ...
    const DealForm = () => (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="seen" className="text-xs text-slate-600">Aku lihat langsung (validasi mata sendiri)</label>
            </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto animate-slide-up">
                ...
                {step === 'select' && <SelectionStep />}
                {step === 'deal' && <DealForm />}
                {step === 'headsup' && <HeadsUpSelection />}
                {step === 'headsup-form' && <HeadsUpForm />}
            </div>
        </div>
    );
}
